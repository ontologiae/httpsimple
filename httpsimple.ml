...
let parse_arg () = match (Array.length Sys.argv)  > 1 with
                | true -> let p = Sys.argv.(1) in
                                ( try 
                                 let prt = int_of_string p in 
                                 pport := prt
                         with e -> failwith (p ^ " .Port must be an integer")
                         )
                | false -> print_endline "Usage : httpsimple -p <port>";;
...
let start () =
  ...
  Unix.bind master_sock (Unix.ADDR_INET(Unix.inet_addr_any, !pport));
